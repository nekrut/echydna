...
class EchyDna(object):
	...
	def __init__(self,lengthOrString,name=None,background=None,seed=None):
		...
		if (length <= 0):
			...
			self.seq = "".join([chooser(background) for _ in range(length)])
	...
	def fasta(self,filename=None,name=None,wrap=None,append=False,returnIt=True):
		...
		if (closeFile):
			f.close()
		return fasta if (returnIt) else None
	...
	def __imul__(self,other):
		"""a*=n replaces a with n copies of itself; if n is negative the reverse complement of a is used"""
		if (isinstance(other,int)):
			pass
		elif (isinstance(other,float)):
			other = int(floor(other))
	...
	def kmers(self,k):
		for ix in range(k,len(self.seq)+1):
	...
	def mutate(self,subRate=None,indelRate=None,indels=None,background=None,seed=None):
		...
		while (ix < len(self.seq)):
			...
			if (indelLen < 0):           # deletion event
				...
				seq += [chooser(background) for _ in range(indelLen)]
